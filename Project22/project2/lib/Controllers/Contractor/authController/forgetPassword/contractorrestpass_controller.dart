...
import 'package:project2/Model/data/Contractor/auth/foregetpassword/contractorrestpass_data.dart';

class ContrctorrestpassController extends GetxController {
  GlobalKey<FormState> key1 = GlobalKey<FormState>();
  GlobalKey<FormState> key2 = GlobalKey<FormState>();
  late TextEditingController newpassword;
  late TextEditingController repassword;
  late String inquiryStatus;
  ContrctorrestpassData contrctorrestpassData =
      ContrctorrestpassData(Get.find());
  ...
  goToSuuccessRestPass() async {
    if (newpassword.text == repassword.text) {
      ...
      var request = await contrctorrestpassData.getRestPasswordData(
          email!, newpassword.text);
      inquiryStatus = respondingRequest(request);
      print(request);
      if (InquiryStatus.success == inquiryStatus) {
        if (request['status'] == "success") {
          Get.offAllNamed("contractorsuccessrestpassword");
        } else {
